...
namespace {
    ...
    IconPosition iconPosition(int x, int y)
    {
        ...
        return x > LOGICAL_SCREEN_WIDTH / 2 ? IconPosition::Left
                                            : IconPosition::Right;
    }
...
int TouchHandler::adjustXForAnimation(int x)
{
    if (x < g_r1)
        return g_r1;
    if (x > LOGICAL_SCREEN_WIDTH - g_r1)
        return LOGICAL_SCREEN_WIDTH - g_r1;
    return x;
}
...
int TouchHandler::adjustYForAnimation(int y)
{
    if (y < g_r1)
        return g_r1;
    if (y > LOGICAL_SCREEN_HEIGHT - g_r1)
        return LOGICAL_SCREEN_HEIGHT - g_r1;
    return y;
}
